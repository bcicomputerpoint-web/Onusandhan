...
import { UploadCloud, FileText, CheckCircle, Clock, Trash2, Edit2, User, RefreshCw, X, Link as LinkIcon, ExternalLink, Mail, Phone, Calendar, MapPin, BookOpen } from 'lucide-react';
import { Button } from '../ui';

// 1. Reusable Summary Card Component
export function SummaryCard({ title, value, icon: Icon, colorClass, bgClass, subtitle }: any) {
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col justify-between hover:border-indigo-200 hover:shadow-md transition-all duration-300 cursor-pointer shadow-sm group">
...
export function UploadCard({ title, icon: Icon, status, fileName, fileUrl, onUpload, onDelete, onReplace, onFileSelect }: any) {
  ...
  return (
    <div 
       className={`bg-white rounded-2xl p-4 border border-slate-200 flex flex-col relative group transition-all duration-300 ease-in-out cursor-pointer h-[120px] shadow-sm hover:shadow-md hover:border-indigo-300 hover:-translate-y-1 ${isDragging ? 'ring-2 ring-indigo-500 bg-indigo-50/50' : ''}`}
       ...
       <div className="mt-auto flex flex-col">
          ...
          {status === 'uploading' ? (
             ...
             <span className="text-[12px] text-slate-400 group-hover:text-indigo-600 transition-colors font-medium mt-0.5">Click or drag file</span>
          )}
       </div>
    </div>
  );
...
export function ProfileCard({ details, onEdit, isAdmin = false }: any) {
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-200 h-full flex flex-col group relative">
       {/* Cover Gradient */}
  ...
  return (
    <div className="bg-white border text-left border-slate-200 rounded-2xl p-4 flex flex-col hover:border-indigo-200 hover:shadow-md transition-all duration-300 ease-in-out group relative overflow-hidden h-full">
       ...
       <div className="flex justify-between items-start mb-3 ml-2">
         <div className="bg-indigo-50 p-2.5 rounded-xl text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-colors shadow-sm">
           <LinkIcon className="w-5 h-5" />
         </div>
         <div className="flex gap-1">
            <a href={link.external_url} target="_blank" className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Visit">
       ...
       <div className="ml-2 flex-1 flex flex-col">
          ...
          <span className="text-[11px] font-bold bg-slate-50 text-slate-500 px-2 py-1 rounded-md w-max mb-3 uppercase tracking-wider">{link.category}</span>
